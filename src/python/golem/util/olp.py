...
def getSubprocess(olpname, id, inp, out, subprocesses, subprocesses_flav, model, use_crossings, conf, ew_index=0):
   ...
   if ew_index == 1:
       ...
       try: 
           ...
           conf.setProperty("olp.alphapower",str(int(conf.getProperty("olp.alphapower"))+2))
           conf.setProperty("olp.alphaspower",str(int(conf.getProperty("olp.alphaspower"))-2))
           if int(conf.getProperty("olp.alphaspower"))<0:
               conf.setProperty("olp.alphaspower",'100')
           qgraf_power = get_qgraf_power(conf)
           conf.setProperty("new_order", qgraf_power)                          
       except:
...
def get_qgraf_power(conf):
   ...
   if alpha_power is None:
      if alphas_power is None:
         ...
         if (correction_type == "QCD" and not qcd_in_ew) and not nolooplevel:
            return [qcd_name, treepower, ipower + 2]
         elif qcd_in_ew:
            return [qcd_name, treepower, ipower + 4]
      ...
      if alphas_power is None:
         ...
         if correction_type == "QCD" or qcd_in_ew:
            ...
            if not nolooplevel:
               if qcd_in_ew:
                 return [qcd_name, treepower, icpower + 4, qed_name, iepower, iepower -2 ]
               else:
...
def process_order_file(order_file_name, f_contract, path, default_conf,
     ...
     for subprocess_number,(lineo,_,_,_) in enumerate(order_file.processes_ordered()):
         ...
         for j in range(0,2):
             ...
             file_ok = golem.util.olp_options.process_olp_options(tmp_contract_file, subconf,
                       ignore_case, ignore_unknown, lineo, quiet=True)
             #if j == 1:
                #subconf.setProperty("olp.alphapower",str(int(subconf["olp.alphapower"])-2))
                #subconf.setProperty("olp.alphaspower",str(int(subconf["olp.alphaspower"])+2))               
       ...
       if conf["olp.correctiontype"]=="EW":
            ...
            for lineno,id, inp, outp in contract_file.processes_ordered():
                ...
                for j in range(0,2):
                    ...
                    subprocesses_conf[id_real].setProperty("main_process", str(subprocess_main))
            ...
            for subprocess in subprocesses.values():
                ...
                try:
                    ...
                    subprocess_conf.setProperty("has_qcd_in_ew",True)
                    if int(subprocess_conf["olp.alphaspower"])<0:
                        subprocess_conf.setProperty("has_qcd_in_ew",False)

                    golem.util.main_misc.generate_process_files(subprocess_conf,
                        from_scratch)  
                    
                    if subprocess_conf.getBooleanProperty("has_qcd_in_ew")==False:
                        for subprocess_main in subprocesses_main.values():
                            if str(subprocess_main) == str(subprocess_conf["main_process"]):
                                subprocess_main.has_ew = False
                                subprocesses_conf_short.append(subprocess_conf)
                                key_list.append(subprocess)
                                #break
                    ...
                    for id in subprocess.getIDs():
                        ...
                        #id_list[id]=channels[id]
                        #print subprocess, id, channels[id]
                ...
                subprocess_conf.setProperty("id_list", id_list)
                subprocesses_conf_short.append(subprocess_conf)
            ...
            for subprocess in subprocesses:
                if subprocesses[subprocess] in key_list:
                    remove_list.append(subprocess)
            for subprocess in remove_list:
                del subprocesses[subprocess]
   ...
   try:
      contract_file.store(f_contract,id_list)
   except IOError as err:

module     olp_module
   ...
   public :: OLP_PrintParameter, OLP_GetProcessnumber
...
contains
   ...
   subroutine OLP_GetProcessnumber(process, nr)&
   & bind(C,name="[%
   @if internal OLP_TO_LOWER %][%
      olp.process_name asprefix=\_ convert=lower %]olp_info[%
   @else %][%
      olp.process_name asprefix=\_ %]OLP_GetProcessnumber[%
   @end @if %][%
   @if internal OLP_TRAILING_UNDERSCORE %]_[%
   @end @if %]")
   use, intrinsic :: iso_c_binding, only: C_CHAR, C_NULL_CHAR[%
   ...
   implicit none   
   character(kind=c_char,len=1), intent(in) :: process
      interface
         function strlen(s) bind(C,name='strlen')
            use, intrinsic :: iso_c_binding
            implicit none
   ...
   l=strlen(process)[%
   @for subprocesses_main prefix=sp. %][% @if is_first %][%
     @for crossings_main include-self prefix=cr. %][%
       @if cr.is_first %]
   if (process(1:l)=="[% cr.inout %]") then [%
       @else %]
   elseif (process(1:l)=="[% cr.inout %]") then[%
       @end @if %]
     nr= [% cr.response %][%
     @end @for %][%
     @else %] [%
...
[% @if eval olp.correctiontype .eq. 'EW' %]
   ...
   @end @if %]")
      ...
      real(kind=c_double), dimension(50), intent(in) :: momenta
      real(kind=c_double), dimension(60), intent(out) :: res
      real(kind=c_double), dimension(60) :: res1, res2
      real(kind=c_double), intent(out) :: acc
      real(kind=c_double) :: acc1, acc2
...
@for subprocesses prefix=sp. %][%
   ...
   subroutine     eval[% cr.id %]([%
      ...
      @if eval olp.correctiontype .eq. 'EW'
      %]
      call ir_subtraction(vecs,real(mu,ki)*real(mu,ki), ir[%
      @select count elements cr.channels
      @case 1 %][%
      @else %], h[%
      @end @select %])[%
